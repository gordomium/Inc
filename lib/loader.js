...
var pullRawContent = function (url, sign, callback) {
    merge.merge('', url, '', function (content) {
        if (sign && signContent(content) !== sign) {
            callback();
            return;
        }
...
var loadAsset = function (options) {
    ...
    if (configure.isStore) {
        ...
        merge.merge(oldUrl, url, moduleContent, function (content) {
            ...
            var caculatedSign = signContent(content)
            if (specifiedSign && caculatedSign !== specifiedSign) {
                reload(2);
                return
            }
            ...
            if (content && version && url && !store.setItem(module, content, version, url, caculatedSign)) {
