...
// 检查签名 （目前只支持 sha1
var checkSign = function (specifiedSign, calculatedSign) {
    return !configure.validateAssetSignIfGiven || !specifiedSign || specifiedSign === calculatedSign
};

// 直接拉取完整资源
var pullRawContent = function (url, sign, callback) {
    merge.merge('', url, '', function (content) {
        if (checkSign(sign, signContent(content))) {
            callback();
            return;
        }
...
var loadAsset = function (options) {
    ...
    if (configure.isStore) {
        ...
        merge.merge(oldUrl, url, moduleContent, function (content) {
            ...
            if (checkSign(specifiedSign, calculatedSign)) {
                reload(2);
                return
            }
