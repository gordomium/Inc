...
// 加载资源
var loadAsset = function (url, module, version, callback) {
    if (loading[url] && !afreshLoaded[url]) {
        if (callback) {
            setTimeout(function () {
                loadAsset(url, module, version, callback);
            }, 5);
            return;
        }
        return;
    ...
    if (configure.isStore) {
        ...
        merge.merge(oldUrl, url, moduleContent, function (content) {
            if (!content && oldUrl !== '') {
                ...
                loadAsset(url, module, null, callback);
                return;
            }
            ...
            if (content && version && url && !store.setItem(module, content, version, url)) {
...
var loadAssets = function (assets, callback) {
    ergodicModules([], assets, function (scripts) {
        ...
        for (var i = 0; i < scripts.length; i++) {
            ...
            var current = loadQueue[moduleName];
            loadAsset(current.path, moduleName, current.version, hook);
        }
    });
};
