 ...
 *                                          'reload:bo-sign', 'reload:bo-content',
 *                                          'full-fetch:bo-content',
 *                                          'store:remove:bo-reload', 'store:remove:bo-content', 'store:remove:bo-eval-illegal',
 *                                          'store:save-failed:bo-illegal', 'reload:exit-bo-ool',
...
var _realReportError = function (e) {
    configure.catch(e, configure)
};
...
var assetCall = function (options) {
    ...
    if (!handle) {
        reportError(new Error('Illegal extname: ' + extname))
        callback();
        return;
    }
    ...
    if (!content) {
        ...
        fetch(url, function (_content) {
            ...
            if (isSignNotValid(sign, signContent(_content))) {
                callback();
                return;
            }
...
var loadAsset = function (options) {
    ...
    if (configure.isStore) {
        ...
        var reload = function (allowRetries) {
            ...
            invokeCount('store:remove:bo-reload')
            store.removeItem(module);

            if (retries[url] >= allowRetries) {
                callback()
                invokeCount('reload:exit-bo-ool')
                return
            }
        ...
        merge.merge(oldUrl, url, moduleContent, function (content, code) {
            if (!content && oldUrl !== '') {
                reload(1);
                reportError(new Error('Merge content got empty'))
                invokeCount('reload:bo-content')
                return;
            }
            ...
            if (isSignNotValid(specifiedSign, calculatedSign)) {
                reload(1);
                reportError(new Error('Merge content sign not match; specified: ' + specifiedSign + ' calculated: ' + calculatedSign))
                invokeCount('sign:invalid')
                invokeCount('reload:bo-sign')
                return
            }
