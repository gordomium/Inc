...
function reportError () {
    ...
    return module.exports.reportError.apply(null, arguments)
}
...
function _getMixDiff (source, target, callback) {
    ...
    _xhr(eRealUrl, function (responseText) {
        if (responseText) {
            ...
            endHook({ mergeCode: -1 })
            reportError(new Error('Diff patch not found'), { source: source, target: target, url: eRealUrl })
            ...
            _xhr(requestUrl, function (body, r) {
                if (/application\/json/i.test(r.getResponseHeader('Content-Type'))) {
                    ...
                    reportError(
                        new Error('Diff server did not response valid content with status: ' + r.status),
                        { body: body, source: source, target: target, url: requestUrl }
                    )
                    endHook({ mergeCode: -1 })
                    callback({
...
function merge (source, target, oldConetnt, callback) {
    ...
    try {
        _getMixDiff(source, target, function (payload) {
            ...
            if (code === RESULT_TYPE.SUCCESS_WITH_DIFF) {
                if (data.length > maxDiffLength) {
                    ...
                    reportError(new Error('Diff is too big'), {
                        lengthLimit: maxDiffLength,
                ...
                reportError(
                    new Error('Illegal merge result code:' + code + '  [fallback to full-fetch]'),
        ...
        reportError(e, { source: source, target: target })
        _xhr(target, function (responseText) {
            callback(responseText, RESULT_TYPE.SUCCESS_WITH_FULL_CONTENT)
        }); 
    }
