...
var _toPatchUrl = function (target, source) {
    return cdnUrl + 
...
var _toFullContentUrl = function (target, source) {
    return serverUrl + 
        '?target=' + 
        target.replace(/[?#].*$/, '') + 
        '&source=' + 
...
var _getMixDiff = function (source, target, callback) {
    ...
    var eRealUrl = isProxy ? target : _toPatchUrl(target, source);
    _xhr(eRealUrl, function (responseText) {
        if (responseText && responseText !== '') {
            ...
            if (!isProxy) {
                _xhr(_toFullContentUrl(target, srouce), function () {
                    return true;
                });
            }
            callback({
...
var merge = function (source, target, oldConetnt, callback) {
    ...
    try {
        _getMixDiff(source, target, function (payload) {
            if (payload.code === 1) {
                callback(_merge(oldConetnt, JSON.parse(payload.data)), 1);
            } else if (payload.code === 2) {
                callback(payload.data, 2);
            } else if (payload.code === 3) {
                callback(oldConetnt, 3);
            } else {
                callback();
            }
        });
